...
class OrderRepositoryImpl implements OrderRepository {
  ...
  Future<Either<Failure, OrderModel>> createOrder({
    ...
    try {
      final response = await _apiClient.post(ApiEndpoints.createOrder, body: {
        'patient_id': patientId,
        'delivery_address': deliveryAddress,
        'delivery_city': deliveryCity,
        'delivery_phone': deliveryPhone,
      ...
      final model = OrderModel.fromJson(response['data']['message']['order']);
      return Right(model);
    } on DioException catch (e) {
      return Left(ServerFailure(e.message ?? 'Server error'));
    } catch (e) {
  ...
  Future<Either<Failure, List<OrderModel>>> getOrders({
    ...
    try {
      final response = await _apiClient.post(ApiEndpoints.getOrders, body: {
        'patient_id': patientId,
        if (status != null) 'status': status,
      });
  ...
  Future<Either<Failure, OrderModel>> getOrderById(String orderId) async {
    try {
      final response =
      await _apiClient.post(ApiEndpoints.getOrderDetail, body: {'order_id': orderId});
      final model = OrderModel.fromJson(response['data']['message']['order']);
      return Right(model);
    } catch (e) {
      return Left(UnknownFailure(e.toString()));
  ...
  Future<Either<Failure, void>> cancelOrder(String orderId) async {
    try {
      await _apiClient.post(ApiEndpoints.cancelOrder, body: {'order_id': orderId});
      return const Right(null);
    } catch (e) {
      return Left(UnknownFailure(e.toString()));
    }
  ...
  Future<Either<Failure, void>> rateOrder({
    ...
    try {
      await _apiClient.post(ApiEndpoints.rateOrder,
          body: {'order_id': orderId, 'rating': rating, 'feedback': feedback});
      return const Right(null);
    } catch (e) {
      return Left(UnknownFailure(e.toString()));
    }
